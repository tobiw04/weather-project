...
To simulate spatial influence and counter potential inaccuracies, the local temperature change is averaged with the changes of its neighbors:


final[t] = average(local[t] + neighbor_local[t])


Since not all stations have four neighbors (e.g. due to being on the edge of the map or too far from other stations), only existing neighbor stations are considered. This results in a smoothed temperature change that reflects surrounding conditions.
